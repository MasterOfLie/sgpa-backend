...
public interface ProcessoRepository extends JpaRepository<Processo, String> {
    @Query("SELECT new dev.matheushenrique.sgpa.dto.ProcessoResponseDTO(p.id, p.description, p.status, st.id, st.name, s.id, s.name, dpt.id, dpt.name, pt.id, pt.anoVigencia, CONCAT( sl.firstName, ' ' ,  sl.lastName)) " +
